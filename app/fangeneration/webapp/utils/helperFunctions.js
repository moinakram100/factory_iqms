        ...
        return new Promise((resolve, reject) => {  // Ensure the function returns a Promise
            if (!oController[dialogId]) {
                ...
                .then(oDialog => {
                    ...
                    oDialog.open();
                    resolve(oDialog); // Resolve with the loaded fragment
                })
                .catch(error => {
                    console.warn("Fragment Loading error :", error);
                    reject(error);
                });
            } else {
                oController[dialogId].open();
                resolve(oController[dialogId]); // Resolve with the existing fragment
            }
        });
    },
