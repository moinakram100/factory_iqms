sap.ui.define([
    ...
    "sap/m/MessageBox",
    "sap/m/MessageToast",
    "com/ingenx/qms/fangenerationfactory/utils/helperFunctions"
  ],
    /**
    ...
    function (Controller, Filter, FilterType, FilterOperator, Fragment, PDF, AUTOTABLE, MessageBox, MessageToast, helperFunctions) {
      ...
      return Controller.extend("com.ingenx.qms.fangenerationfactory.controller.Material", {
        onInit: async function () {
          ...
          oRouter.getRoute("RouteMaterialFanGeneration").attachPatternMatched(this.onObjectMatched, this);
        ...
        fetchSTODialogOk: async function () {
          let oTable = this.byId("_FetchSTOTable");
          let aSelectedIndices = oTable.getSelectedIndices();
  
          if (!aSelectedIndices.length) {
            this.fetchSTODialog.close();
            return;
          }
  
          let StockOrder = "", sQuantity, sUom, materialSelected, mFlag = false;
  
          aSelectedIndices.forEach(function (iIndex) {
            let oContext = oTable.getContextByIndex(iIndex);
  
            salesOrder = oContext.getProperty("Sto");
            sQuantity = parseFloat(oContext.getProperty('Quantity'));
            sUom = oContext.getProperty('Unit');
            materialSelected = oContext.getProperty('Material');
          });
  
          // Show confirmation dialog before proceeding
          let that = this;
          sap.m.MessageBox.confirm("Are you sure you want to proceed with the selected Stock Order?", {
            actions: [sap.m.MessageBox.Action.OK, sap.m.MessageBox.Action.CANCEL],
            onClose: function (oAction) {
              if (oAction === sap.m.MessageBox.Action.OK) {
  
                that.processStockOrderAssign(StockOrder, materialSelected, mFlag, sQuantity, sUom);
  
                that.byId("_FetchSTOTable").clearSelection();
              }
  
            }
          });
        },
        processStockOrderAssign: async function (Stockorder, materialSelected, mFlag, sQuantity, sUom) {
          let oTable = this.byId("_IDGenTable2");
          let aSelectedIndices = oTable.getSelectedIndices();
  
          let parkingNo, vehicleNo, Plant, Driver, Cleaner, material;
          aSelectedIndices.forEach(function (iIndex) {
            let oContext = oTable.getContextByIndex(iIndex);
            parkingNo = oContext.getProperty("ParkingNo");
            vehicleNo = oContext.getProperty("VehicleNumber");
        ...
        fetchSTOapi: async function (matnr, Plant) {
          const oModel = this.getOwnerComponent().getModel();
          const oView = this.getView();
          const oJSONModel = new sap.ui.model.json.JSONModel([]);
          // Define UI5 filters
          const oAndFilter = new sap.ui.model.Filter({
            filters: [
              new sap.ui.model.Filter("Material", sap.ui.model.FilterOperator.EQ, matnr),
              new sap.ui.model.Filter("Plant", sap.ui.model.FilterOperator.EQ, Plant)
            ],
            and: true // This ensures AND logic between filters
          });
  
          try {
            // Construct the binding path with the correct $filter
            const sPath = `/xIQMSxfetch_sto`;
            const oListBinding = oModel.bindList(sPath);
  
            const aContexts = await oListBinding.filter(oAndFilter).requestContexts(0, Infinity);
  
            // Extract data from contexts
            const aData = aContexts.map(oContext => oContext.getObject());
  
            console.log("Fetched STO Data:", aData);
            oJSONModel.setData(aData);
            oView.setModel(oJSONModel, "fetchSTOModel");
  
          } catch (oError) {
            console.error("Error fetching STO data:", oError);
  
            oJSONModel.setData([]); // Set empty on failure
            oView.setModel(oJSONModel, "fetchSTOModel");
          }
        },
  
        fetchSTODialogCancel: function () {
          this.fetchSTODialog.close();
          this.byId('_IDGenTable2').clearSelection();
          this.byId('_FetchSTOTable').clearSelection();
        },
        ...
        FetchSo: async function () {
          ...
          if (STOValidation) {
            sap.m.MessageBox.error("Stock Order already created.");
            return;
          }
          ...
          helperFunctions._openValueHelpDialog(this, 'fetchSoDialog', 'com.ingenx.qms.fangenerationfactory.fragments.FetchSo');
  
  
        },
        FetchSTO: async function () {
          if (!ParkingNoValidation) {
            sap.m.MessageBox.warning("Please select Parking No");
            return;
          }
          ...
          if (STOValidation) {
            sap.m.MessageBox.error("Stock Order already created for this entry.");
            return;
          }
          const oTable = this.byId("_IDGenTable2");
          const aSelectedIndices = oTable.getSelectedIndices();
          ...
          aSelectedIndices.forEach(function (iIndex) {
            ...
            matnr = oContext.getProperty("Product"); // Corrected property path
            Plant = oContext.getProperty("Plant"); // Corrected property path
  
          });
          console.log("matcode", matnr)
          console.log("Plant", Plant)
          if (!Plant) {
            sap.m.MessageToast.show("Assign the Plant First.")
            return
          }
  
          const fetchSTO = await this.fetchSTOapi(matnr, Plant);
          // console.log("fetchMat", fetchMat)
  
          helperFunctions._openValueHelpDialog(this, 'fetchSTODialog', 'com.ingenx.qms.fangenerationfactory.fragments.FetchSTO');
        ...
        printFan: async function () {
          ...
          let rowData = [];
          let matcode, shipToParty, parkingNo, vehicleNo, Destination, salesOrder, DeliveryNo, ShipmentNo, Quantity, Stockorder;
          ...
          for (let i = 0; i < aSelectedIndices.length; i++) {
            ...
            salesOrder = oContext.getProperty("SalesOrder") || "N/A";
            DeliveryNo = oContext.getProperty("DeliveryNo") || "N/A";
            ShipmentNo = oContext.getProperty("ShipmentNo") || "N/A";
            Quantity = oContext.getProperty("Quantity") || "N/A";
            Stockorder = oContext.getProperty("Stockorder") || "N/A";
  
            rowData.push([salesOrder, DeliveryNo, Stockorder, matcode, Quantity]);
          }
  
          const filterDriverName = mergedData.filter(data => data.ParkingNo === parkingNo);
          const driverName = filterDriverName.length > 0 ? filterDriverName[0].Driver : "N/A";
          ...
          // Filling Advise Note in gray
          doc.setFontSize(16);
          doc.setTextColor(128); // gray
          ...
          doc.text("Blank", startXValue, startY + 5 * lineHeight);
  
          // Table
          const tableStartY = startY + 7 * lineHeight;
          doc.autoTable({
            head: [['Sales Order', 'Delivery No.', 'STO No.', 'Material', 'Qty']],
            body: rowData,
            startY: tableStartY
          });
          ...
          // Row 1 - Authorized and Driver Signature
          ...
          // Row 2 - Security and Gate Signature
        ...
        pressCreateSo: async function () {
          ...
          try {
            ...
            let oDialog = await helperFunctions._openValueHelpDialog(this, 'assignMaterialQuantity_dialog', 'com.ingenx.qms.fangenerationfactory.fragments.AssignMaterialQty');
        ...
        onSubmitMaterialQty: function () {
          ...
          if (this.flagSTO) {
            this.CreateSTOfn(sUom, qtyFloat, sMaterial);
          }
        },
        ...
        CreateSTOfn: async function (sUom, sQuantity, selectedMaterial) {
          console.log("STO create Pressed");
          ...
          try {
            ...
            let matcode, Soldtoparty, parkingNo, Vehicle, Driver, Cleaner, Plant, PlantText;
            let oTable = this.byId("_IDGenTable2");
            let aSelectedIndices = oTable.getSelectedIndices();
            ...
            aSelectedIndices.forEach(function (iIndex) {
              ...
              Cleaner = oContext.getProperty("Cleaner");
              Plant = oContext.getProperty("Plant");
              PlantText = oContext.getProperty("PlantText");
            });
            ...
            let payloadSTO = {
              "po_create": "",     // e.g 4500000736  will be genearated
              "create_tohead": [
                {
                  "PoNumber": "",
                  "CompCode": "AJPL",
            ...
            console.log("STO Payload ", payloadSTO);
  
            let oModel = this.getOwnerComponent().getModel();
  
            // payload for scheduling entry
            let newPayload = {
              "Parkingno": parkingNo,
              "DeliveryNo": "",
              "ShipmentNo": "",
            ...
            // STO creation
            let oBindList4 = oModel.bindList("/stocreateSet");
            oBindList4.create(payloadSTO, true);
            ...
            oBindList4.attachCreateCompleted(async (oEvent) => {
              try {
                ...
                if (params.success) {
                  let response = params.context.getObject();
                  console.log('responseSTO', response);
  
                  let oPo_create = response.po_create;
                  // pushing SO
  
                  let returnMessages = response.so_nav_return || [];
                  let errorMessages = returnMessages.filter(msg => msg.Type === "E");
  
                  if (errorMessages.length > 0 || !response.po_create) {
                    let errorText = errorMessages.map(msg => msg.Message).join("\n") || "STO number is missing from backend response.";
                    throw new Error("STO creation failed:\n" + errorText);
                  }
                  if (oPo_create) {
  
                    newPayload.Stockorder = oPo_create;
                  }
  
                  sap.m.MessageBox.show("Sales Order " + oPo_create + " created", {
                    title: "Success",
                    icon: sap.m.MessageBox.Icon.SUCCESS,
                    actions: [sap.m.MessageBox.Action.OK],
                  });
                  ...
                  await oBindListItem.filter(aFilterItem).requestContexts().then(function (aContexts) {
                    if (aContexts.length > 0) {
                      ...
                      aContexts[0].setProperty("Stotime", time);
                      aContexts[0].setProperty("Stodate", createdDate);
                    } else {
                      console.warn("No matching entries found in park_itemSet2 for ParkingNo update.");
                    }
                  });
                ...
                console.error("Error in create completion:", error);
                sap.m.MessageBox.error(error.message || "An error occurred while processing the STO.");
              } finally {
                sap.ui.core.BusyIndicator.hide();
              }
            });
            ...
            console.error("CreateSTO failed:", error);
            sap.m.MessageBox.error(error.message);
          } finally {
            sap.ui.core.BusyIndicator.hide();
            this.byId('_IDGenTable2').clearSelection();
        ...
        createDelivery: async function () {
          ...
          try {
            ...
            if (success) {
              await this.updateDeliveryStatus(deliveryContext);
            }
        ...
        validateDeliveryInputs: function () {
          if (
            !ParkingNoValidation &&
            !DeliveryNoValidation &&
            !shipmentNoValidation &&
            !statusValidation &&
        ...
        prepareAndCreateDelivery: async function () {
          const oTable = this.byId("_IDGenTable2");
          let parkingNo, Vehicle, salesOrder, Quantity, Stockorder;
          ...
          oTable.getSelectedIndices().forEach(iIndex => {
            ...
            Stockorder = oContext.getProperty("Stockorder");
          });
  
          let payload, entitySet;
          if (STOValidation) {
            entitySet = "/stodeliverySet";
            payload = {
              Delivery: "",
              NumDeliveries: "",
              sto_headitem: [{
          ...
          let oModel = this.getOwnerComponent().getModel();
          let oBindList = oModel.bindList(entitySet);
          sap.m.MessageToast.show("Creating Delivery ...");
  
          return new Promise((resolve, reject) => {
            const ctx = oBindList.create(payload, true);
        ...
        updateDeliveryStatus: async function (context) {
          const response = context.getObject();
          const DelivNumbResponse = response.DelivNumb || response.Delivery;
  
  
          const currentDate = new Date().toISOString().split('T')[0];
          const createdDate = `${currentDate}T00:00:00Z`;
          const now = new Date();
          const time = now.toTimeString().split(" ")[0];
          console.log("Delivery No, date and time", DelivNumbResponse, createdDate, time);
          ...
          let oBindListItem = oModel.bindList("/park_itemSet2");
          let aFilterItem = new sap.ui.model.Filter("ParkingNo", sap.ui.model.FilterOperator.EQ, ParkingNoValidation);
          ...
          if (DelivNumbResponse) {
            let schedulingkey;
            if (STOValidation) {
              schedulingkey = `SalesOrder='${SalesOrderValidation}',Stockorder='${STOValidation}'`;
            } else {
              schedulingkey = `SalesOrder='${SalesOrderValidation}',Stockorder='${STOValidation}'`;
            }
  
            const contextBinding = oModel.bindContext(`/SchedulingSet(${schedulingkey})`, null, { $$updateGroupId: "myUpdateGroup" });
            const obj = await contextBinding.requestObject();
  
            if (obj) {
              const contextSched = contextBinding.getBoundContext();
              contextSched.setProperty('DeliveryNo', DelivNumbResponse);
              await oModel.submitBatch("myUpdateGroup");
            }
          }
  
          // ✅ Now refresh the table here (since status is updated)
          setTimeout(() => {
            sap.m.MessageToast.show("Refreshing table data ...");
        ...
        showSuccessAndRefresh: function (context) {
          const response = context.getObject();
          const DelivNumb = response.DelivNumb || response.Delivery;
          const returnMessages = response.del_nav_return || response.sto_toreturn || [];
          ...
          if (errorMessages.length > 0 || !DelivNumb) {
            ...
            sap.m.MessageBox.error("Delivery creation failed:\n" + errorText);
            return false;
          }
        ...
        //     if (!ParkingNoValidation && (!SalesOrderValidation && !STOValidation) && !DeliveryNoValidation && !shipmentNoValidation && !statusValidation) {
        //       sap.m.MessageBox.error("Please Select the Parking Number.");
        //       return;
        //     } else if (!SalesOrderValidation) {
        //       sap.m.MessageBox.error("Please create SO / STO first.");
        //       return;
        ...
        //     const contextBinding = oModel.bindContext(`/SchedulingSet('${salesOrder}')`, null, { $$updateGroupId: "myUpdateGroup" });
        //     const obj = await contextBinding.requestObject();
        ...
        //     return
        //   }
        //   helperFunctions._openValueHelpDialog(this, 'createShipmentDialog', 'com.ingenx.qms.fangenerationfactory.fragments.ShipToTas');
        ...
        //       ShipTo = oContext.getProperty("CustomerCode");
        //     });
        ...
        //           let aContexts = await oBindListitem.filter(aFilteritem).requestContexts();
        //           if (aContexts.length > 0) {
        //             aContexts[0].setProperty("Status", "LAN Created");
        //             aContexts[0].setProperty("LanTime", time);
        //             aContexts[0].setProperty("LanDate", createdDate);
        //           }
  
        //           let oBindList = oModel.bindList("/park_so_documentSet");
        //           let aFilter = new sap.ui.model.Filter("ParkingNo", sap.ui.model.FilterOperator.EQ, parkingNo);
  
        //           let aContextsSo = await oBindList.filter(aFilter).requestContexts();
        //           if (aContextsSo.length > 0) {
        //             aContextsSo[0].setProperty("ShipmentNo", Shnumber);
        //           }
        ...
        onUpdate: function () {
          helperFunctions._openValueHelpDialog(this, 'updateLicenseDialog', 'com.ingenx.qms.fangenerationfactory.fragments.UpdateLicence');
        ...
        onSelectTable: function (oEvent) {
          ...
          if (aSelectedData.length === 0) {
            ...
            STOValidation = aSelectedData[0].Stockorder;
        ...
        onCustomerPress: function () {
          ...
          helperFunctions._openValueHelpDialog(this, 'assignCustomerDialog', 'com.ingenx.qms.fangenerationfactory.fragments.AssignCustomer');
  
        },
        onPlantPress: function () {
          if (!ParkingNoValidation) {
            sap.m.MessageBox.warning("Please select Parking No");
            return;
          }
          helperFunctions._openValueHelpDialog(this, 'assignPlantDialog', 'com.ingenx.qms.fangenerationfactory.fragments.AssignPlant');
        ...
        onCreateVehicleMaster: function () {
          ...
          helperFunctions._openValueHelpDialog(this, 'VehicleMasterDialog', 'com.ingenx.qms.fangenerationfactory.fragments.CreateVehicleMaster');
          setTimeout(() => {
        ...
        onPlantCode: function () {
          helperFunctions._openValueHelpDialog(this, 'PlantCodeDialog', 'com.ingenx.qms.fangenerationfactory.fragments.PlantCode');
        ...
        onCustomerCode: function () {
          ...
          helperFunctions._openValueHelpDialog(this, 'CustomerCodeDialog', 'com.ingenx.qms.fangenerationfactory.fragments.CustomerCode');
        ...
        onPlantClose: function (oEvent) {
          ...
          this.byId("plantInput_id").setValue(oSelectedItem.getTitle());
          CustCode = this.getView().byId("plantInput_id").getValue();
          ShipToName = oSelectedItem.getDescription();
          // Destination = oSelectedItem.getInfo();
        ...
        onSubmitPlant: function () {
          ...
          this.getView().byId("plantInput_id").setValue("")
          const oTable = this.byId("_IDGenTable2");
          oTable.clearSelection();
          this.assignPlantDialog.close();
        ...
        onParkingNumber: function () {
          helperFunctions._openValueHelpDialog(this, 'parkingNoDialog', 'com.ingenx.qms.fangenerationfactory.fragments.ParkingNo');
        ...
        onVehicleNumber: function () {
          helperFunctions._openValueHelpDialog(this, 'VehicleNoDialog', 'com.ingenx.qms.fangenerationfactory.fragments.VehicleNo');
        ...
        pressGoBtn: function () {
          ...
          let productControl = oView.byId("FANMaterialProductSelect"); // Select control
