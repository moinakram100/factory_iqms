sap.ui.define([
    ...
    function (Controller, Filter, FilterType, FilterOperator, Fragment, PDF, AUTOTABLE, MessageBox, MessageToast, helperFunctions) {
      ...
      return Controller.extend("com.ingenx.qms.fangenerationfactory.controller.Solid", {
        onInit: async function () {
          ...
          oRouter.getRoute("RouteSolidFanGeneration").attachPatternMatched(this.onObjectMatched, this);
        ...
        printFan: async function () {
          ...
          let matcode, shipToParty, parkingNo, vehicleNo, Destination, salesOrder, DeliveryNo, ShipmentNo, Quantity;
          const oTable = this.byId("_IDGenTable2");
          let aSelectedIndices = oTable.getSelectedIndices();
  
          let rowData = []; // Array to store table data for the PDF
          let that = this;
  
          let fetchPromises = []; // Array to store promises for vehicle master data
          ...
          for (let i = 0; i < aSelectedIndices.length; i++) {
            ...
            // Store promise for fetching vehicle data
  
            // fetchPromises.push(that.fetchVehicleMasterData(vehicleNo).then(vehicleData => {
            //   let Quantity = vehicleData.length > 0 ? Number(vehicleData[0].VehMaxvol) : "N/A";
            //   rowData.push([salesOrder, DeliveryNo, ShipmentNo, matcode, Quantity]);
            // }));
          ...
          // Wait for all vehicle master data fetch operations to complete
          // await Promise.all(fetchPromises);
  
          console.log("rowData", rowData);
  
          let filterDriverName = mergedData.filter(function (data) {
            return data.ParkingNo == parkingNo;
          });
  
          let driverName = filterDriverName.length > 0 ? filterDriverName[0].Driver : "N/A";
  
          console.log("filterDriverName", filterDriverName);
          ...
          const startY = 40;  // Adjusted to leave space for the heading
          const lineHeight = 10;
  
          // Adding heading text at the top
          doc.setFontSize(18);  // Set font size for heading
          doc.text("Filling Advise Note", 105, 20, { align: "center" });  // Centered heading
          ...
          doc.autoTable({
            head: [['Sales Order', 'Delivery No.', 'Shipment No.', 'Material', 'Qty']], // Table header
            body: rowData, // Table data (rows)
            startY: startY + 7 * lineHeight // Start position of the table (below the text)
          });
            ...
            oBindList4.attachCreateCompleted(async (oEvent) => {
              try {
                ...
                if (params.success) {
                  ...
                  let aFilter = new sap.ui.model.Filter("ParkingNo", sap.ui.model.FilterOperator.EQ, salesOrder);
        ...
        onInputChange: function () {
          ...
          SubmitTuVehicleMaster: function () {
            ...
            let tuNumber = this.getView().byId("TUnumber_id").getValue()
            let self = this;
            ...
            sap.m.MessageBox.success("Transport Unit and Vehicle Master Created", {
              onClose: function (oAction) {
                if (oAction === sap.m.MessageBox.Action.OK) {
                  ...
                  setTimeout(function () { self.onObjectMatched() }, 3000);
        ...
        //     // this.onVehicleMasterDialog.close();
