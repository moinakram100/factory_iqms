sap.ui.define([
  ...
  "com/ingenx/qms/parkingfactory/utils/helperFunctions",
  "sap/m/MessageToast"
],
  function (Element,Controller,MessageBox,Filter,FilterOperator,MaterialType,helperFunctions, MessageToast) {
    "use strict";
    let MatCode
    let ShipToName
    let Destination
    let oView;
    return Controller.extend("com.ingenx.qms.parkingfactory.controller.CreateParking", {
      onInit: function () {
      ...
      onTransportCode: function () {
        helperFunctions._openValueHelpDialog(this, 'transporterDialog', 'com.ingenx.qms.parkingfactory.fragments.TransportCode');
      ...
      onCustomerCode: function () {
        ...
        if (transactionType === "Sales Order") {
          helperFunctions._openValueHelpDialog(this, 'CustomerCodeDialog', 'com.ingenx.qms.parkingfactory.fragments.CustomerCode');
        } else {
          helperFunctions._openValueHelpDialog(this, 'PlantCodeDialog', 'com.ingenx.qms.parkingfactory.fragments.PlantCode');
      ...
      getStatusDescValue: async function (status) {
        const oModel = this.getOwnerComponent().getModel();
        const oBindList = oModel.bindList("/SecurityStatus");
        try {
          const oContexts = await oBindList.requestContexts(0, Infinity);
          const oData = oContexts.map(context => context.getObject());      
          const matchedItem = oData.find(item => item.MessageText === status);
          return matchedItem?.MsgNo || null;
        } catch (error) {
          console.error("Error in getStatusDescValue:", error);
          return null;
        }
      },      
      ...
      onSubmit: async function () {
        try {
          this.radioSelectedText = "Loading" 
          ...
          let stausNum =await this.getStatusDescValue(parkStatus);
          ...
          if (oData && oData.value && oData.value.length > 0) {
            ...
            //  Note : Passing  MaterialCode instead of Product ( material) description
              ...
              let parkingData = {parkingGate, parkingArea, createdDate, vehicleNumber, driverName, cleanerName, sSelectedText, TransportCode, customerCode, MatCode, VehicleType, parkStatus, plant, plantText,stausNum}
              this.createParkingFn(parkingData);
            }
      ...
      createParkingFn: async function (parkingData) {
        let {parkingGate, parkingArea, createdDate, vehicleNumber, driverName, cleanerName, sSelectedText, TransportCode, customerCode, MatCode, VehicleType, parkStatus, plant, plantText,stausNum} = parkingData
        try {
          let now = new Date();
          let isoDate = now.toISOString().split("T")[0]; 
          var sFormattedTime = now.getHours().toString().padStart(2, '0') + ":" +
          ...
          let oPayload = {
            ...
            ParkTime: sFormattedTime,
            ParkingNo: "",
            ParkingGate: parkingGate,
            ParkingArea: parkingArea,
            VehicleNumber: vehicleNumber,
            Status: stausNum,
            StatusDesc: parkStatus,
            park_tostatus: [
              {
                Parkingno: "",
                ...
                Bayno: this.bayNo || "",
                Plant: plant,
                Description: ShipToName ? ShipToName : plantText,
      ...
      onScanSuccess: async function (oEvent) {
        ...
        if (oEvent.getParameter("cancelled")) {
          ...
          if (scanResult) {          
            ...
            this.unit = scannedData.Uom
            this.quantity = scannedData.qty
            this.bayNo = scannedData.BayNo
            this.createdDate = scannedData.createdDate
            this.createdTime = scannedData.createdTim
