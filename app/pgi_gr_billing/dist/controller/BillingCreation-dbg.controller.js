...
sap.ui.define([
    ...
    function (Controller, MessageBox) {
      ...
      return Controller.extend("com.ingenx.qms.pgigrbilling.controller.BillingCreation", {
  
        onInit: async function () {
          oView = this.getView();
  
          let oRouter = sap.ui.core.UIComponent.getRouterFor(this);
          oRouter.getRoute("RouteBillingCreation").attachMatched(this._onObjectMatched, this);
        ...
        createBilling: async function () {
          ...
          try {
            ...
            try {
              ...
              let contextArr = aContexts.filter( context => context.getObject().Parkingno === ParkingNoValidation );
              if (contextArr.length) {
                
                contextArr[0].setProperty("BillingNo", oVbeln);
              ...
              let aHeaderContexts = await oBindListHeader.filter(aFilter).requestContexts();
              if (aHeaderContexts.length === 0) throw new Error("No parking data record found.");
  
              if (aHeaderContexts.length > 0) {
                aHeaderContexts[0].setProperty("Status", "Invoiced");
              ...
              onClose: async function (oAction) {
                if (oAction === sap.m.MessageBox.Action.OK) {
                  try {
                    ...
                    // Success Message
                    sap.m.MessageBox.success("Status Updated Successfully.", {
                      onClose: function () {
